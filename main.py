...
def register_account(domain_choice, name_option="1", gender_option="3", custom_pass=None, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Check if account created successfully (has c_user cookie)
            if "c_user" in login_coki:
            ...
            # FIXED: Better OTP detection - check for confirmation page
            # Facebook sends OTP to email when account needs verification
            otp_keywords = [
                "checkpoint", "confirm", "code", "enter the code", 
                "verification", "we sent a code", "email", "verify",
            ...
            # Also check if there's a form with code field
            soup = BeautifulSoup(response_text, 'html.parser')
            form_element = soup.find('form')
            if form_element:
                for inp in form_element.find_all('input'):
                    name = inp.get('name', '')
                    if 'code' in name.lower() or 'confirm' in name.lower():
                        needs_otp = True
                        break
            ...
            # If we reach here, something went wrong - retry
            print(f"[DEBUG] Registration not successful for {email}, retrying...")
