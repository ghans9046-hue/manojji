...
_executor = ThreadPoolExecutor(max_workers=10)  # Reduced for stability
...
otp_sessions = {}  # {user_id: {"session": session, "email": email, "name": name, "password": password, "count_info": {}}}
    ...
    if not mine:
        ...
        for i, acc in enumerate(mine, 1):
            ...
            lines.append(
                ...
                f"    🔑 `{acc['password']}`\n"
                f"    🆔 `{acc['uid']}`\n"
                f"    🍪 `{acc.get('cookies', 'N/A')[:50]}...`{by_line}"
            )
        body = "\n\n".join(lines)
...
async def cb_stop(callback: types.CallbackQuery):
    ...
    if callback.from_user.id != uid and callback.from_user.id != OWNER_ID:
        ...
        return
    stop_flags[uid] = True
    # Also clear any pending OTP session for this user
...
# Handle OTP responses from users
@dp.message()
async def handle_otp(message: types.Message):
    uid = message.from_user.id
    entered = message.text.strip()
    
    # Check if user has an active OTP session
    if uid in otp_sessions:
        session_data = otp_sessions[uid]
        
        # Validate OTP format (5-6 digits)
        if entered.isdigit() and len(entered) in [5, 6]:
            await message.answer("⏳ *Verifying OTP...*", parse_mode="Markdown")
            
            # Submit OTP to Facebook
            success = fb.submit_otp(session_data["session"], entered)
            ...
            if success:
                ...
                # Save to created accounts
                created_accounts.append(account_data)
                save_users()
                
                # Send full account details with cookies
                await message.answer(
                ...
                # Continue with remaining accounts if batch creation
                count_info = session_data.get("count_info", {})
                if count_info:
                    await continue_batch_creation(uid, count_info)
                else:
                    # No more accounts, show menu
                    await bot.send_message(
                        uid,
                        "🤖 *Facebook Auto Creator (Yandex Email)*\n\nSelect options step by step 👇",
                        parse_mode="Markdown",
    ...
    # If not OTP session, check if waiting for input
    ...
    if awaiting == "count":
        ...
        await start_batch_creation(uid, count_info, message.chat.id)

async def start_batch_creation(uid, count_info, chat_id):
    """Start batch account creation with OTP handling"""
    stop_flags[uid] = False
    
    banner = await bot.send_message(
        chat_id,
        f"⚡ *Creating {count_info['total']} account(s)...*\n\n"
        f"📧 Using Yandex email: `{YANDEX_EMAIL}`\n"
        f"💡 You'll be asked for OTP for each account.\n\n"
        f"Results will appear after each verification 👇",
        parse_mode="Markdown",
        reply_markup=make_stop_kb(uid)
    )
    creating_msg[uid] = banner.message_id
    
    count_info["completed"] = 0
    
    for i in range(count_info["total"]):
        ...
        result = fb.register_account_with_otp(
            ses,
            name_option=count_info["name_option"],
            gender_option=count_info["gender_option"],
            custom_pass=count_info["password"]
        )
        
        if result is None:
            await bot.send_message(
                chat_id,
                f"❌ *Account {i+1}/{count_info['total']} Failed*\n\n"
                f"Could not complete registration. Retrying...",
                parse_mode="Markdown"
            )
            continue
        
        if result.get("uid"):
            # Account created without OTP needed
            count_info["completed"] += 1
            ...
            await bot.send_message(
                chat_id,
                f"✅ *Account {count_info['completed']}/{count_info['total']} Created!*\n\n"
                f"👤 *Name:* `{result['name']}`\n"
            ...
            # Need OTP verification - store session and wait for user
            otp_sessions[uid] = {
                "session": ses,
                "email": result["email"],
                "name": result["name"],
                ...
                "count_info": {
                    ...
                    "completed": count_info["completed"],
                    "name_option": count_info["name_option"],
                    "gender_option": count_info["gender_option"],
                    "password": count_info["password"]
                }
            }
            
            await bot.send_message(
                chat_id,
                f"📧 *OTP Required for Account {i+1}/{count_info['total']}*\n\n"
                f"👤 *Name:* `{result['name']}`\n"
                f"📧 *Yandex Email:* `{result['email']}`\n\n"
                f"🔐 *Please check your Yandex inbox and send the 5-6 digit OTP code here.*\n\n"
                f"_(The OTP will arrive within 30 seconds)_",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="🛑 Cancel All", callback_data=f"stop:{uid}")]
                ])
            )
            # Wait for OTP - the handle_otp function will continue
            return
    ...
    await bot.send_message(
        chat_id,
        f"🎉 *Done!* {count_info['completed']}/{count_info['total']} accounts created.{credits_summary}",
        parse_mode="Markdown"
    )
    await bot.send_message(
        chat_id,
...
async def continue_batch_creation(uid, count_info):
    """Continue batch creation after OTP verification"""
    count_info["completed"] += 1
    
    if count_info["completed"] < count_info["total"]:
        ...
        if result is None:
            await bot.send_message(
                uid,
                f"❌ *Account {count_info['completed']+1}/{count_info['total']} Failed*\n\n"
                f"Could not complete registration. Continuing...",
                parse_mode="Markdown"
            )
            await continue_batch_creation(uid, count_info)
            return
        ...
        if result.get("uid"):
            ...
            await bot.send_message(
                uid,
                f"✅ *Account {count_info['completed']+1}/{count_info['total']} Created!*\n\n"
                f"👤 *Name:* `{result['name']}`\n"
                f"📧 *Email:* `{result['email']}`\n"
                f"🔑 *Password:* `{result['password']}`\n"
                f"🆔 *UID:* `{result['uid']}`\n"
            ...
            await bot.send_message(
                uid,
                f"📧 *OTP Required for Account {count_info['completed']+1}/{count_info['total']}*\n\n"
                f"👤 *Name:* `{result['name']}`\n"
                f"📧 *Yandex Email:* `{result['email']}`\n\n"
                f"🔐 *Please check your Yandex inbox and send the 5-6 digit OTP code here.*",
                parse_mode="Markdown"
        ...
        await bot.send_message(
            uid,
            f"🎉 *All Done!* {count_info['completed']}/{count_info['total']} accounts created.{credits_summary}",
            parse_mode="Markdown"
        )
        await bot.send_message(
            uid,
