...
async def handle_otp(message: types.Message):
    ...
    if awaiting == "count":
        ...
        banner = await message.answer(
            ...
            f"📧 Using Yandex email with plus addressing\n"
            f"💡 You'll be asked for OTP for each account if needed.",
            parse_mode="Markdown",
        ...
        if success_count == 0:
            ...
            await message.answer(
                f"🎉 *Done!* {success_count}/{count} accounts created.{credits_summary}",
                parse_mode="Markdown"
            )
...
async def main():
    ...
    print(f"📧 Using Yandex Email for plus addressing")
